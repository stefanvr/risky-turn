...
`$SCRATCHPAD` below is this session's scratchpad directory, named in your
environment. If none is named, make one with `mktemp -d` and use that.

```bash
# Start AFTER the user approves the companion. --open auto-opens their browser on
# the first screen; --project-dir persists mockups and enables same-port restart.
# Pass this session's scratchpad directory, never the project root — a sketch is
# transient, and the repository holds decided truth only.
bash scripts/start-server.sh --project-dir "$SCRATCHPAD" --open

# Returns: {"type":"server-started","port":52341,
#           "url":"http://localhost:52341/?key=ab12…",
#           "screen_dir":"$SCRATCHPAD/.superpowers/brainstorm/12345-1706000000/content",
#           "state_dir":"$SCRATCHPAD/.superpowers/brainstorm/12345-1706000000/state"}
```
...
**Note:** Pass **this session's scratchpad directory** as `--project-dir`, not the project root. Mockups then persist in `$SCRATCHPAD/.superpowers/brainstorm/` and survive server restarts within the session, while the repository stays free of sketch files — a sketch settles nothing durable, and anything it does settle lands in the artifact that owns it. Without the flag entirely, files go to `/tmp` and the port changes on restart, which breaks the user's open tab.
...
bash scripts/start-server.sh --project-dir "$SCRATCHPAD" --open --foreground
```
...
If the session used `--project-dir`, mockup files persist under that directory for later reference in this session. Only `/tmp` sessions get deleted on stop.
